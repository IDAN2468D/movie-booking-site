...
export default function PersonalInfoSettings() {
  ...
  return (
    <div className="space-y-12">
      <div className="bg-gradient-to-br from-white/[0.05] to-transparent rounded-[40px] p-12 border border-white/10 shadow-[0_40px_80px_-20px_rgba(0,0,0,0.5)] relative overflow-hidden backdrop-blur-3xl">
        <div className="flex flex-col md:flex-row items-center gap-8 mb-10 pb-8 border-b border-white/5">
          ...
          <div className="text-center md:text-right flex-1">
            <h3 className="text-3xl font-black text-white mb-2 font-display uppercase tracking-tighter">פרופיל <span className="text-primary">מחובר</span></h3>
            <p className="text-base text-slate-400 font-medium">הפרטים שלך מסונכרנים בבטחה עם חשבון ה-Google שלך</p>
        ...
        <h3 className="text-2xl font-black text-white mb-10 flex items-center gap-4 uppercase font-display tracking-tight">
        ...
        <div className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-3">
              <label className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mr-2">שם מלא</label>
              <input
                type="text"
                defaultValue={session?.user?.name || ''}
                className="w-full bg-black/40 border border-white/10 rounded-2xl p-5 text-lg text-white font-medium focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary/50 transition-all text-right shadow-inner"
              />
            </div>
            <div className="space-y-3">
              <label className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mr-2">שם תצוגה</label>
              <input
                type="text"
          ...
          <div className="space-y-3">
            <label className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mr-2">כתובת אימייל</label>
            <div className="relative">
              ...
              <input
                ...
                disabled
                className="w-full bg-white/5 border border-white/5 rounded-2xl py-5 pr-14 pl-5 text-lg text-slate-500 font-medium cursor-not-allowed text-right opacity-70"
              />
            </div>
          </div>

          <div className="space-y-3">
            <label className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mr-2">טלפון</label>
            <input
              type="tel"
          ...
          <div className="pt-6 flex justify-start items-center gap-6">
            <button 
              ...
              disabled={saving}
              className="px-10 py-5 bg-primary text-black rounded-2xl font-black text-sm uppercase tracking-[0.2em] shadow-[0_15px_30px_rgba(255,159,10,0.3)] hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50 disabled:scale-100 border border-transparent hover:border-white/20"
            >
...
function ToggleRow({ title, desc, enabled }: { title: string; desc: string; enabled: boolean }) {
  ...
  return (
    <div className="flex items-center justify-between p-8 rounded-[24px] bg-black/40 border border-white/10 hover:border-white/20 transition-colors shadow-inner group">
      <div>
      ...
      <button onClick={() => setOn(!on)} className="relative shrink-0">
