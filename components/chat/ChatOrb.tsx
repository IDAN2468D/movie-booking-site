...
export default function ChatOrb({ onClick, botState }: ChatOrbProps) {
    ...
    switch (botState) {
      ...
      case 'speaking':
        return {
          ...
          gradient: 'from-primary/40 to-pink-500/20',
  ...
  return (
    ...
    >
      ...
      {/* Thinking state: spinning outer highlight ring */}
      <motion.div
        animate={botState === 'thinking' ? { rotate: 360 } : { rotate: 0 }}
        transition={botState === 'thinking' ? { repeat: Infinity, duration: 1.5, ease: 'linear' } : { duration: 0.5 }}
        className={cn(
          "absolute inset-1 border border-transparent rounded-full",
          botState === 'thinking' ? "border-t-cyan-400 border-r-cyan-400/50" : "border-white/10"
        )}
      />
