...
import {
  useDiscoveryIsOverTarget,
  useDiscoveryFocalScale,
...
export function NeuralDiscoveryDashboard({ userId }: { userId: string }) {
  ...
  const getGenreByBubbleId = (bubbleId: string): string => {
    if (bubbleId === "intense") return "action";
    if (bubbleId === "thoughtful") return "sci-fi";
    if (bubbleId === "dark") return "horror";
    return "comedy";
  };

  const handleDrag = (event: any, info: PanInfo, bubble: typeof EMOTION_BUBBLES[0]) => {
    ...
    if (isIntersecting && hoveredCoreId !== bubble.id) {
      ...
      playSweep(getGenreByBubbleId(bubble.id));
      setOverTarget(true);
      setFocalScale(1.25);
  ...
  const handleDragEnd = async (event: any, info: PanInfo, bubble: typeof EMOTION_BUBBLES[0]) => {
    if (!coreRef.current) return;
    const coreRect = coreRef.current.getBoundingClientRect();
    const bubbleRect = (event.target as HTMLElement).getBoundingClientRect();
    const isIntersecting = !(
  ...
  return (
    <div className="min-h-[90vh] bg-[#05070B] text-white font-['Inter',_sans-serif] p-4 md:p-8 overflow-hidden relative" dir="rtl">
      ...
      <div className="relative z-10 max-w-7xl mx-auto flex flex-col h-full">
        ...
        {errorMsg && <div className="mb-8 p-4 rounded-xl bg-red-900/30 border border-red-500/50 text-red-200 text-center">{errorMsg}</div>}
        ...
        <div className="flex-1 flex flex-col lg:flex-row items-center justify-center gap-12 lg:gap-24 mb-16">
          <div className="flex flex-wrap justify-center lg:flex-col gap-6 lg:w-1/3">
            {EMOTION_BUBBLES.map((bubble) => (
              <motion.div
                ...
                drag
                dragSnapToOrigin
                onDrag={(e, info) => handleDrag(e, info, bubble)}
                onDragEnd={(e, info) => handleDragEnd(e, info, bubble)}
                whileHover={{ scale: 1.05 }}
                whileDrag={{ scale: 1.15, zIndex: 50 }}
                className={`cursor-grab active:cursor-grabbing px-6 py-4 rounded-full bg-gradient-to-r ${bubble.color} border border-white/[0.12]`}
                style={{ boxShadow: `0 10px 30px -10px ${bubble.shadow}, inset 0 0 0 1px rgba(255, 255, 255, 0.15)` }}
              >
                <span className="font-['Outfit'] font-bold text-white text-lg drop-shadow-md pointer-events-none">{bubble.label}</span>
              </motion.div>
            ))}
          </div>
          ...
          <div className="flex items-center justify-center lg:w-2/3 relative h-[300px] md:h-[400px]">
            ...
            <motion.div
              ...
              className={`w-64 h-64 md:w-80 md:h-80 rounded-full border border-white/[0.15] flex items-center justify-center relative backdrop-blur-[20px] transition-all duration-500 transform-gpu ${isAbsorbing || isPending || isOverTarget ? "shadow-[0_0_80px_rgba(0,240,255,0.6)] border-[#00F0FF]" : "shadow-[0_25px_50px_-12px_rgba(0,0,0,0.7)]"}`}
              style={{
                scale: focalScale,
                willChange: "transform, opacity",
                background: isAbsorbing || isPending || isOverTarget ? "radial-gradient(circle, rgba(0,240,255,0.2) 0%, rgba(5,7,11,0.6) 70%)" : "rgba(5, 7, 11, 0.4)",
              }}
            >
              <motion.div
                animate={{ scale: isPending ? [1, 1.2, 1] : [1, 1.05, 1], opacity: isPending ? [0.5, 1, 0.5] : [0.3, 0.5, 0.3] }}
              ...
              <div className="text-center z-10 pointer-events-none">
                {isPending ? (
                  <span className="font-['Outfit'] font-bold text-[#00F0FF] text-xl animate-pulse">מסנכרן תודעה...</span>
                ) : activeMood ? (
                  <span className="font-['Outfit'] font-bold text-[#00F0FF] text-xl">{activeMood}</span>
                ) : (
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-[#05070B] via-[#05070B]/50 to-transparent flex flex-col justify-end p-5">
                    <div className="flex items-center justify-between mb-3">
                      <span className="px-3 py-1 bg-[#00F0FF]/20 border border-[#00F0FF]/50 text-[#00F0FF] text-xs font-bold rounded-full">{movie.collection}</span>
                      <span className="text-[#00F0FF] font-mono text-sm font-bold">{movie.match}% התאמה</span>
                    </div>
