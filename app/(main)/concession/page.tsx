...
import { BioAcousticConcessionContainer } from "@/src/components/concessions/BioAcousticConcessionContainer";
...
export default function ConcessionPage() {
  const [viewMode, setViewMode] = useState<'express' | 'bio' | 'holographic' | 'grid'>('express');
  ...
  return (
    <main className="fixed inset-0 bg-neutral-950 pt-16 px-4 md:px-8 pb-4 overflow-y-auto flex flex-col z-40">
      ...
      <div className="max-w-7xl mx-auto w-full flex-1 flex flex-col space-y-4 relative z-10 min-h-0 pb-28" dir="rtl">
        {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/10 pb-3 shrink-0">
          <div className="space-y-1">
            ...
            <p className="text-neutral-400 font-inter text-xs md:text-sm max-w-2xl hidden sm:block">
              חוויה קולינרית רפרקטיבית המשלבת עוזר ביו-אקוסטי 35Hz, תפריט אקספרס מהיר והתאמת קומבו חכמה.
            </p>
          </div>

          {/* View Mode Switcher */}
          <div className="flex items-center gap-1.5 p-1 rounded-2xl bg-white/5 border border-white/10 shrink-0 self-start sm:self-auto overflow-x-auto">
            <button
              onClick={() => setViewMode('express')}
              className={`px-3 py-1.5 rounded-xl text-xs font-outfit font-bold transition-all ${
                viewMode === 'express'
                  ? 'bg-gradient-to-r from-amber-500 to-orange-500 text-neutral-950 shadow-[0_0_12px_rgba(245,158,11,0.4)]'
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-outfit font-medium transition-all ${
                viewMode === 'bio'
