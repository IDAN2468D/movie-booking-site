...
import { ActorRoleEmotionGraph } from "@/components/actor/ActorRoleEmotionGraph";
import { ActorAudioSceneSynthesizer } from "@/components/actor/ActorAudioSceneSynthesizer";
import { ActorFanBadgeShard } from "@/components/actor/ActorFanBadgeShard";
    ...
    <div className="min-h-screen bg-[#05070B] text-right font-['Assistant',_'Rubik',_sans-serif]" dir="rtl">
      ...
      <section className="relative w-full h-[60vh] md:h-[70vh] overflow-hidden">
        ...
        <div className="absolute top-8 end-8 z-30">
          <Link
            href="/"
            className="flex items-center justify-center w-12 h-12 rounded-2xl bg-neutral-950/40 backdrop-blur-xl border border-white/20 text-white hover:border-[#00F0FF]/50 transition-all shadow-xl"
          >
            <ArrowRight size={24} />
          </Link>
        </div>
          ...
          <div className="flex-1">
            {profile.trending && (
              <div className="inline-flex items-center space-x-2 space-x-reverse mb-4 bg-[#FFB800]/20 border border-[#FFB800]/50 rounded-full px-4 py-1.5 shadow-[0_0_15px_rgba(255,184,0,0.2)]">
                <span className="w-2.5 h-2.5 rounded-full bg-[#FFB800] animate-pulse"></span>
                <span className="text-[#FFB800] text-sm font-black tracking-widest uppercase font-['Outfit']">שחקן מבוקש</span>
              </div>
            )}
            <h1 className="text-5xl md:text-7xl font-black text-white leading-tight drop-shadow-2xl font-['Outfit']">
              {profile.name}
            </h1>
      ...
      <main className="max-w-7xl mx-auto px-8 md:px-16 py-12 md:py-20 grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="p-8 md:p-12 rounded-[2rem] backdrop-blur-[40px] saturate-[250%] bg-neutral-950/40 border border-white/[0.12] shadow-2xl">
          ...
          {/* Gemini AI Role Emotion Graph */}
          <ActorRoleEmotionGraph 
            actorName={profile.name}
            filmography={profile.filmography?.map((f: { filmTitle: string }) => f.filmTitle) || []}
          />

          {/* Web Audio 35-50Hz Sub-Bass Synthesizer */}
          <ActorAudioSceneSynthesizer actorName={profile.name} />
        ...
        <div className="space-y-8">
          {/* Fan Badge Shard */}
          <ActorFanBadgeShard actorName={profile.name} />
          ...
          {profile.notableRoles && profile.notableRoles.length > 0 && (
            <div className="p-8 rounded-[2rem] backdrop-blur-[40px] bg-neutral-950/40 border border-white/[0.12] shadow-2xl">
              ...
              <ul className="space-y-4 font-['Inter']">
                {profile.notableRoles.map((role: string, idx: number) => (
                  <li key={idx} className="flex items-center gap-4 group cursor-default">
                    <div className="w-2 h-2 rounded-full bg-[#00F0FF]/50 group-hover:bg-[#00F0FF] transition-colors"></div>
          ...
          {/* Quick Links Card */}
          <div className="p-8 rounded-[2rem] bg-gradient-to-br from-[#00F0FF]/10 to-transparent border border-[#00F0FF]/30 text-center shadow-2xl">
            ...
            <Link 
              href="/"
              className="inline-block w-full py-3 rounded-xl bg-black/40 border border-[#00F0FF]/50 text-[#00F0FF] hover:bg-[#00F0FF]/20 transition-all font-bold font-['Outfit']"
            >
              הצג קטלוג קולנוע
            </Link>
          </div>
