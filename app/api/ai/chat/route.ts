...
export async function POST(req: NextRequest) {
  try {
    ...
    const systemPrompt = `
      ...
      2. **הזמנה אוטומטית**: אם המשתמש רוצה להזמין סרט ספציפי, ספק לו קישור ישיר והשתמש בתגית מיוחדת: [ACTION:BOOK:MOVIE_ID]. הסבר לו שהמערכת תפתח לו את מפת המושבים באופן אוטומטי.
      3. **רכישת כרטיסים**: אם המשתמש שואל איך קונים, הסבר שניתן לבצע רכישה מאובטחת. אם הוא בחר סרט, הצע לו להשתמש בתגית [ACTION:PURCHASE:MOVIE_ID] כדי לעבור ישירות לדף התשלום.
      4. **חיפוש סרטים**: אם הוא מחפש סרט שלא קיים ברשימה ה"חמים", הצע לו לחפש בשורת החיפוש העליונה.
      5. אם המשתמש אומר "כן" או "אשמח" אחרי שהצעת לבדוק שעות, בדוק איזה סרט הוזכר אחרון והצע שעות דמיוניות (למשל: 19:30, 21:00, 22:45) באולם IMAX או VIP.
      6. שמור על סגנון "Liquid Glass" - מודרני, נקי ומתקדם.
